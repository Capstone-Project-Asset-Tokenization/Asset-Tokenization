...
export let createAssetObjFromContract = async (contractItem) => {
  ...
  assetObj.images = await Promise.all(
    contractItem["9"].map(async (item) => {
      return await item;
    })
  );
  assetObj.supportingDocuments = await Promise.all(
    contractItem["10"].map(async (item) => {
      return await item;
    })
  );
...
export let createUserObjFromContract = async (contractItem) => {
  console.log("user data from contarct", contractItem);
  let userObj = {};
