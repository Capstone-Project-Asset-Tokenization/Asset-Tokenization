...
import { CustomError, NotFoundError } from "../utils/error/customErrors";
...
export default class UserServie {
    ...
    async register(user: RegistrationInput) {
        ...
        if (user.phoneNumber)
            existingUser = await this.userRepository.getUserByPhoneNumber(user.phoneNumber)
        ...
        if(user.walletAddress)
            existingUser = await this.userRepository.getUserByWalletAddress(user.walletAddress)
    ...
    async login(email: string, password: string) {
        ...
        return generateToken({ _id: user._id, email: user.email, roles: user.roles, walletAddress: user.walletAddress })
    }
    ...
    async getUsersByWalletAddresses(walletAddresses: string[]) {
        let users = await this.userRepository.getUsersByWalletAddresses(walletAddresses)
        return users
    }

    async updateUserWalletAddress(id: string, walletAddress: string) {
        const existingUser = await this.userRepository.getUserByWalletAddress(walletAddress)
        if (existingUser) {
            throw new CustomError(409, 'User wallet address already exists')
        }
